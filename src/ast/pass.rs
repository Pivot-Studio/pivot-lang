...
pub fn run_immix_pass(module: &Module) {
    ...
    // add global _GC_MAP_
    let gcmap_name = "_LLVM_StackMaps".to_string();
    let stack_map = module.add_global(ctx.i8_type(), Default::default(), &gcmap_name);
    stack_map.set_linkage(Linkage::External);
    // println!("gcmap_name: {}", gcmap_name);
    MAP_NAMES
        .inner
        .lock()
        .borrow_mut()
        .push(gcmap_name.clone() +  module.get_source_file_name().to_str().unwrap() + "__init");
    let ptr_tp = ctx.i8_type().ptr_type(AddressSpace::from(0));
    ...
    let mod_init_f = module.add_function(
        &(gcmap_name +  module.get_source_file_name().to_str().unwrap() + "__init"),
