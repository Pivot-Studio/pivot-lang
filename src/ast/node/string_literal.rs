...
impl Node for StringNode {
    fn emit<'a, 'b>(
        ...
        let len = builder.build_struct_gep(alloca, 1, "len", &tp.borrow(), ctx).unwrap();
        let byte_len = builder.build_struct_gep(alloca, 2, "byte_len", &tp.borrow(), ctx).unwrap();
        let read_arr = builder.build_struct_gep(alloca, 3, "real_arr_str", &tp.borrow(), ctx).unwrap();
